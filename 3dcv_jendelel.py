...
class Network:
    ...
    def construct(self, hidden_layer_size):
        with self.session.graph.as_default():
            with tf.name_scope("inputs"):
                ...
                self.input_keep_p = tf.placeholder_with_default(1.0, [], name="input_keep_prob") 
                self.hidden_keep_p = tf.placeholder_with_default(1.0, [], name="hidden_keep_prob") 

            flattened_images = tf_layers.flatten(self.images, scope="preprocessing")
            dropout_input = tf.nn.dropout(flattened_images, self.input_keep_p)
            hidden_layer = tf_layers.fully_connected(dropout_input, num_outputs=hidden_layer_size, activation_fn=tf.nn.relu, scope="hidden_layer")
            dropout_hidden = tf.nn.dropout(hidden_layer, self.hidden_keep_p)
            output_layer = tf_layers.fully_connected(dropout_hidden, num_outputs=self.LABELS, activation_fn=None, scope="output_layer")
    ...
    def train(self, images, labels, input_keep_prob=1, hidden_keep_prob=1, summaries=False, run_metadata=False):
        ...
        args = {"feed_dict": {self.images: images, self.labels: labels, self.input_keep_p:input_keep_prob, self.hidden_keep_p : hidden_keep_prob}}
...
if __name__ == "__main__":
    ...
    results = []
    for input_p in [0.8, 0.9, 1]:
        for hidden_p in [0.8, 0.9, 1]:

            exp_name = "Dropout: input_keep_probability: {}, hidden_keep_probability {}".format(input_p, hidden_p)
            # Load the data
            ...
            for i in range(args.epochs):
                while mnist.train.epochs_completed == i:
                    ...
                    network.train(images, labels, input_p, hidden_p, network.training_step % 100 == 0, network.training_step == 0)
