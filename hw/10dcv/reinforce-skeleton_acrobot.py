...
class PolicyGradient:
    def __init__(self, observations, policy_network, learning_rate, threads=1, seed=42):
        ...
        with self.session.graph.as_default():
            ...
            losses = tf.mul(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, self.chosen_actions), self.rewards)
            self.training = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss=tf.reduce_mean(losses))
