...
class Network:
    def __init__(self, alphabet_size, rnn_cell, rnn_cell_dim, embedding_dim, logdir, expname, threads=1, seed=42):
        ...
        with self.session.graph.as_default():
            ...
            outputs_flat = tf_layers.fully_connected(outputs, 2)
            print("outputs_flat", outputs_flat.get_shape())
            outputs_argmax = tf.argmax(outputs_flat, 2)
            print("outputs_argmax", outputs_argmax.get_shape())

            seq_mask = tf.sequence_mask(self.sentence_lens)
            labels_vec = tf.boolean_mask(self.labels, seq_mask)
            print("labels_vec", labels_vec.get_shape())
            outputs_vec = tf.boolean_mask(outputs_argmax, seq_mask)
            print("outputs_vec", outputs_vec.get_shape())

            self.predictions = tf.cast(outputs_vec, tf.int64)
            print("predictions", self.predictions.get_shape())
            loss = tf_losses.sparse_softmax_cross_entropy(tf.cast(outputs_argmax, tf.float32), self.labels)
            # loss = tf.reduce_sum((tf.cast(outputs_vec, tf.float32)-tf.cast(labels_vec, tf.float32))**2)
