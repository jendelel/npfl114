...
class Network:
    ...
    def __init__(self, rnn_cell, rnn_cell_dim, num_words, num_chars, logdir, expname, threads=1, seed=42, word_embedding=100, char_embedding=100, keep_prob=0.5, num_filters=512, l2=0.001):
        ...
        with self.session.graph.as_default():
            ...
            ### LAYER 1
            c1 = self._1d_conv(sliced, num_filters, [7, char_embedding], stride=[2, char_embedding], padding='SAME')
            print("c1", c1.get_shape())
            l = tf_layers.max_pool2d(c1, kernel_size=[3, 1], stride=[2, 1], padding='SAME')
            for block_i in [6,12,24]:
                for conv_i in range(block_i):
                    l = addLayer(l)
                    print("l", l.get_shape())
                l = addTransition(l)
            for i in range(16):
