...
class Network:
    ...
    def __init__(self, rnn_cell, rnn_cell_dim, num_words, num_chars, logdir, expname, threads=1, seed=42, word_embedding=100, char_embedding=100, keep_prob=0.5, num_filters=512, l2=0.001):
        ...
        with self.session.graph.as_default():
            ...
            _, (state_fw, state_bw) = tf.nn.bidirectional_dynamic_rnn(rnn_cell_co, rnn_cell_co, inputs,
                                                                      self.sentence_lens, dtype=tf.float32,
                                                                      scope="rnn_words")

            states_concat = tf.concat(1, [state_fw, state_bw])
            print("states_concat_word", states_concat.get_shape())

            sentence_embeddings = tf.nn.embedding_lookup(states_concat, self.sentence_ids)
            print("sentence_embeddings", sentence_embeddings.get_shape())

            _, (state_fw, state_bw) = tf.nn.bidirectional_dynamic_rnn(rnn_cell_co, rnn_cell_co, sentence_embeddings,
            ...
            output_layer = tf_layers.fully_connected(states_concat, num_outputs=self.LANGUAGES)
