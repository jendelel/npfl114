...
class Network:
    ...
    def __init__(self, rnn_cell, rnn_cell_dim, num_words, num_chars, logdir, expname, threads=1, seed=42, word_embedding=100, char_embedding=100, keep_prob=0.5, num_filters=512, l2=0.001, loss_gamma=0.01):
        ...
        with self.session.graph.as_default():
            ...
            self.prompts = tf.placeholder(tf.int32, [None])
            self.levels = tf.placeholder(tf.int32, [None])
            ...
            output_prompts = tf_layers.fully_connected(dropout, num_outputs=self.PROMPTS)
            prompts_loss = tf_losses.sparse_softmax_cross_entropy(output_prompts, self.prompts)
            output_levels = tf_layers.fully_connected(dropout, num_outputs=self.LEVELS)
            levels_loss = tf_losses.sparse_softmax_cross_entropy(output_levels, self.levels)

            train_vars = tf.trainable_variables()
            lossL2 = tf.add_n([tf.nn.l2_loss(v) for v in train_vars])*l2
            self.loss = loss = tf_losses.sparse_softmax_cross_entropy(output_layer, self.languages) + lossL2 + loss_gamma * levels_loss + loss_gamma * prompts_loss
    ...
    def train(self, sentence_lens, word_ids, charseq_ids, charseqs, charseq_lens, languages, levels, prompts):
        _, summary = \
            self.session.run([self.training, self.summary],
                             {self.sentence_lens: sentence_lens, self.word_ids: word_ids,
                              ...
                              self.levels: levels, self.prompts: prompts})
        self.summary_writer.add_summary(summary, self.training_step)

    def evaluate(self, sentence_lens, word_ids, charseq_ids, charseqs, charseq_lens, languages, levels, prompts, dataset):
        accuracy, loss, summary = \
            self.session.run([self.accuracy, self.loss, self.summary],
                             {self.sentence_lens: sentence_lens, self.word_ids: word_ids,
                              self.charseq_ids: charseq_ids, self.charseqs: charseqs, self.charseq_lens: charseq_lens,
                              self.languages: languages, self.levels: levels, self.prompts: prompts,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--word_embedding", default=200, type=int, help="word_embedding")
    ...
    parser.add_argument("--loss_gamma", default=0.1, type=float, help="loss gamma")
    ...
    expname = "{}-{}{}-bs{}-epochs{}-char{}-word{}-nf{}-l2:{}-gamma{}".format(tools.exp_name(__file__), args.rnn_cell, args.rnn_cell_dim, args.batch_size, args.epochs, args.char_embedding, args.word_embedding, args.num_filters, args.l2, args.loss_gamma)
    network = Network(rnn_cell=args.rnn_cell, rnn_cell_dim=args.rnn_cell_dim,
                      ...
                      l2=args.l2, loss_gamma=args.loss_gamma)
    ...
    for epoch in range(args.epochs):
        ...
        while not data_train.epoch_finished():
            ...
            network.train(sentence_lens, word_ids, charseq_ids, charseqs, charseq_lens, languages, levels, prompts)

        sentence_lens, word_ids, charseq_ids, charseqs, charseq_lens, tags, levels, prompts, languages = \
            data_dev.whole_data_as_batch()
        dev_accuracy, dev_loss = network.evaluate(sentence_lens, word_ids, charseq_ids, charseqs, charseq_lens, languages, levels, prompts, "dev")
