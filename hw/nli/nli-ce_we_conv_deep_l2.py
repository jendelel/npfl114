...
class Network:
    ...
    def __init__(self, rnn_cell, rnn_cell_dim, num_words, num_chars, logdir, expname, threads=1, seed=42, word_embedding=100, char_embedding=100, keep_prob=0.5, num_filters=512, l2=0.001):
        ...
        with self.session.graph.as_default():
            ...
            ### LAYER 2
            c2_1 = self._1d_conv(c1, num_filters, [3, 1], stride=[1, 1], padding='SAME')
            c2_2 = self._1d_conv(c2_1, num_filters, kernel_size=[3,1], stride=[1,1], padding='SAME')
            pool2 = tf_layers.max_pool2d(inputs=c2_2, kernel_size=[3, 1], stride=[2, 1], padding='SAME')
            ### LAYER 3
            c3_1 = self._1d_conv(pool2, num_filters*2, kernel_size=[3,1], stride=[1,1], padding='SAME')
            c3_2 = self._1d_conv(c3_1, num_filters*2, kernel_size=[3,1], stride=[1,1], padding='SAME')
            pool3 = tf_layers.max_pool2d(inputs=c3_2, kernel_size=[3, 1], stride=[2,1], padding='SAME')
            ### LAYER 4
            c4_1 = self._1d_conv(pool3, num_filters*4, kernel_size=[3,1], stride=[1,1], padding='SAME')
            c4_2 = self._1d_conv(c4_1, num_filters*4, kernel_size=[3,1], stride=[1,1], padding='SAME')
            pool4 = tf_layers.max_pool2d(inputs=c4_2, kernel_size=[3, 1], stride=[3,1], padding='SAME')
            ### LAYER 5
            c5_1 = self._1d_conv(pool4, num_filters*8, kernel_size=[3,1], stride=[1,1], padding='SAME')
            c5_2 = self._1d_conv(c5_1, num_filters*8, kernel_size=[3,1], stride=[1,1], padding='SAME')
            pool5 = tf_layers.max_pool2d(inputs=c5_2, kernel_size=[8, 1], stride=[8,1])

            print("pool", pool5.get_shape())
            ...
            fc1 = tf_layers.fully_connected(dropout1, num_outputs=4096, activation_fn=tf.nn.relu)
            dropout2 = tf_layers.dropout(fc1, keep_prob=keep_prob, is_training=self.is_training)
            fc2 = tf_layers.fully_connected(dropout2, num_outputs=2048, activation_fn=tf.nn.relu)
            dropout3 = tf_layers.dropout(fc2, keep_prob=keep_prob, is_training=self.is_training)
            output_layer = tf_layers.fully_connected(dropout3, num_outputs=self.LANGUAGES, activation_fn=None)
